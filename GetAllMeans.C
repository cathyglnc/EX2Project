...
Int_t fill_sim (const Int_t det_sim, const Int_t scale_sim){
    ...
    //Double_t fac_res = 0.5; //Test
    //Double_t fac_res = 1.5; //Test
    //Double_t fac_res = 0.25; //Test
...
//    f_fit->SetNpx ((fit_high - fit_low)*ene_g_bin/(ene_g_high - ene_g_low)); // binning from the simulations
    f_fit->SetParameters (offset, slope, 1., 2.e3, 5.);
...
void GetAllMeans(){
    ...
    tree->Branch("ratio",ratio,Form("ratio[%i]/D",duration-start));
    tree->Branch("offset",offset,Form("offset[%i]/D",duration-start));
