...
void fit_selfRadio (){
    ...
    const Long64_t my_start = 1.*3600.e9; // h
    const Long64_t my_span = 1.*3600.e9; // 1 h
    const Double_t fit_low = 0.3; // MeV
    const Double_t fit_high = 1.7;
    
    // crude calibration (checking the h_qdc_g in c_hist)
    peak1 = 10600;
    peak2 = 20100;
    slope = (1.46 - 0.79)/(peak2 - peak1);
    offset = 1.46 - slope*peak2;
    ...
    cout<<" offset : "<< offset << " slope : " << slope << endl;
    f_fit->SetParameters (offset, slope, 1., 5.e3, 5.);
    for (Int_t i = 0; i < 2; i++) f_fit->SetParLimits (i, 0., 1.e6);
    ...
    for (Long64_t i = 0; i < f_exp->GetEntries ()/1.; i++)
    {
        ...
        if (plug_g == my_qdc)
        {
            if (stamp_g > my_start && stamp_g < my_start + my_span) h_ene_g[1]->Fill (f_fit->GetParameter (1)*qdc_g + f_fit->GetParameter (0));
        }
    }
